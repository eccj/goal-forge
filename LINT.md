...
| 4 | **Assumption authority** | Explicit "make reasonable assumptions, list them in the report; nothing waits on the user" — EXCEPTION: a §RED-HOLD terminal HOLD on an irreversible agent-unauthorized action is an ALLOWED terminal state, not a wait | items that require future user input (a genuine §RED-HOLD hold does NOT count against this) |
