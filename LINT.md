...
| 4 | **Assumption authority** | Explicit "make reasonable assumptions, list them in the report; nothing waits on the user" — EXCEPTION: a §RED-HOLD on an irreversible agent-unauthorized action is an ALLOWED terminal state, not a wait | items that require future user input (a genuine §RED-HOLD hold does NOT count against this) |
...
| 7 | **Independence from user** | Zero items blocked on user decisions; leftovers moved to "decide before launching"; EXCEPTION: a §RED-HOLD is a legitimate terminal state, not a blocking dependency | "user will explain X later" inside the goal |
