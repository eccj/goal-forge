...
ASSUMPTION: on ambiguity assume + list in report; never wait on the user — EXCEPT §RED-HOLD (irreversible unauthorized action, e.g. key rotation): name it, ledger a HELD entry, STOP once. Baseline = semgrep+njsscan / gitleaks / osv-scanner; swaps → decide-list.
...
1) Target path: assumed the current repo root containing package-lock.json; confirm if the API is one package inside a monorepo (then siblings' in-scope status). 2) Scanner swaps: baseline is semgrep+njsscan/gitleaks/osv-scanner — user may substitute ESLint-security, Snyk, Trivy, or npm audit (note: npm audit is network-bound and breaks J1 offline reproducibility). 3) Pinned CVE-DB snapshot id+date to use for osv-scanner --offline (reproducibility anchor). 4) Severity policy: assumed report-only (findings triaged, none gate the run) — decide if HIGH/CRITICAL should hard-fail. 5) Tribunal models (Q6): assumed prosecutor=Opus, jurors default Sonnet with J3=Opus for the semantic Goodhart call; J1/J2 can drop to the Haiku cheap lane (checklist briefs) for the mechanical re-runs. 6) Live-secret handling: assumed masked-fingerprint-only in the ledger and key rotation deferred to a §RED-HOLD HOLD the user executes — confirm the rotation owner/runbook.
