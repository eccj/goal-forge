...
FORBIDDEN: deploy to a REMOTE/prod cluster or push a public registry (local kind + `kind load` only; =§RED-HOLD) · commit secrets/kubeconfig/tokens · `:latest` deployed ref · other kube-contexts · edit lint/CI/helm scripts to force green.
ASSUMPTION: on ambiguity assume + list it; never wait on the user. Prefilled: service = minimal Go HTTP `/healthz`+`/`; CI validated locally (actionlint+act, no `git push`). EXCEPT §RED-HOLD (prod deploy / external push): name it, ledger a HELD entry + the exact user command, STOP once.
