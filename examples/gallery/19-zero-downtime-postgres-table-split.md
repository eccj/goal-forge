...
□ D5 Contract (§RED-HOLD HOLD): the prod DROP of T's moved columns/old table is irreversible, NOT self-authorized — evidence: an E-D5 HELD entry naming the exact `DROP TABLE`/`ALTER TABLE T DROP COLUMN…` AND the user command to run after the dual-write soak; STOP once.
...
ASSUMPTION: on ambiguity assume + list it; never wait — except the §RED-HOLD D5 HOLD. Stack: Docker Postgres 16 + raw SQL via psql; migration tool + trigger-vs-app dual-write: decide-before-launch swaps.
