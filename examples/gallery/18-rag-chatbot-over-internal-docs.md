...
FORBIDDEN: writing prod docs or indexing outside the corpus dir · any network call during eval (offline only) · fitting thresholds AFTER results · leaking eval.jsonl into any prompt/few-shot · deploy/publish (→§RED-HOLD) · scope creep.
ASSUMPTION: stack = local embeddings + FAISS + temp=0 seeded gen, eval offline vs cached outputs; on ambiguity choose+list, never wait — EXCEPT §RED-HOLD irreversible (prod deploy / hosted-index upload): name it, ledger a HELD entry, STOP once.
...
1) Exact internal-docs corpus (repo/dir/wiki path + access) that defines the "corpus dir" boundary. 2) Numeric bars: recall@5 floor, groundedness floor, hallucination ceiling, answerable-accuracy floor, K (max cites/answer), k (retrieval top-k). 3) Eval-set sizes N (answerable) / M (unanswerable) and who authors the gold chunk-ids + gold answers (SME vs synthetic). 4) Generation LLM + whether local or a hosted API — a hosted call breaks the offline/cached-reproducibility assumption and makes any hosted-index upload a §RED-HOLD hold. 5) Groundedness scoring method (NLI model vs lexical overlap); note an LLM-judge would violate the offline-eval constraint. 6) Confirm the assumed stack (embeddings/vector store/generation) or run the bakeoff. 7) Is a chat UI/endpoint in scope or is CLI-only acceptable for D2's live query? 8) Tribunal juror models (assumed sonnet default, opus for the hardest numeric re-run).
