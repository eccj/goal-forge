...
ASSUMPTION: on ambiguity assume + list it; never wait — EXCEPT §DAL-C (e.g. a Web-Store PUBLISH): name it, ledger a HELD entry with the exact user command, STOP once. Stack: vanilla TS+esbuild+vitest+Puppeteer, no framework — recorded.
